...
const ContactEditorItem = ({ index, itemData, onUpdateFull, onRemove, onMoveUp, onMoveDown, isFirst, isLast, isExpanded, onToggle }: any) => {
  const defaultItem = { text: { mr: '', en: '' }, icon: 'MapPin' };
  const itemHist = useBlockHistory(defaultItem, itemData, (newItemData: any) => onUpdateFull(index, newItemData));
  const currentItem = itemHist.value;
  ...
  return (
    <div className="p-2 border border-slate-200 rounded-lg bg-slate-50 mb-3">
      <EditorBlockHeader
        title={currentItem.text?.mr || currentItem.text?.en || `Contact Item ${index + 1}`}
        isExpanded={isExpanded}
        onToggle={onToggle}
        history={itemHist}
        rightAction={
...
export const HeroFeaturesTimelineLayoutEditor: React.FC<HeroFeaturesTimelineLayoutEditorProps> = ({ data, updateData, blockId, expandedSection }) => {
  ...
  // Auto-migrate legacy contactInfo to contactList if needed
  if (!safeData.contactList || safeData.contactList.length === 0) {
    if (safeData.contactInfo && (safeData.contactInfo.address || safeData.contactInfo.phone || safeData.contactInfo.email)) {
      const migrated = [];
      if (safeData.contactInfo.address) {
        migrated.push({
          icon: 'MapPin',
          text: typeof safeData.contactInfo.address === 'string'
            ? { en: safeData.contactInfo.address, mr: safeData.contactInfo.address }
      ...
      if (safeData.contactInfo.phone) {
        migrated.push({
          icon: 'Phone',
          text: { en: safeData.contactInfo.phone, mr: safeData.contactInfo.phone }
        });
      }
      if (safeData.contactInfo.email) {
        migrated.push({
          icon: 'Mail',
          text: { en: safeData.contactInfo.email, mr: safeData.contactInfo.email }
        });
      }
      if (migrated.length > 0) {
        safeData.contactList = migrated;
      }
    }
  }
  ...
  const defaultContactHeader = { title: { mr: '', en: '' }, icon: '' };
  const contactHeaderHist = useBlockHistory(defaultContactHeader, safeData.sectionHeaders?.contactInfo, (newD: any) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, contactInfo: newD }));
                  ...
                  {safeData.hero?.heroImage ? (
                    <>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/20">
                        <span className="bg-white/90 text-slate-700 text-xs font-semibold px-2 py-1 rounded shadow-sm">Click to change image</span>
                      </div>
                    </>
                  ) : (
  ...
  if (activeSection === 'contactInfo') {
    return (
      <div className="pb-10 space-y-4">
                  ...
                  <div className="p-4 space-y-4 bg-white border-t border-slate-200 mt-2 rounded-b-lg">
                    <div>
                      ...
                      <PhoneticInput 
                        ...
                        onChange={(val) => { const n = JSON.parse(JSON.stringify(contactHeaderHist.value)); if(!n.title) n.title = {}; n.title.mr = val; contactHeaderHist.update(n); }} 
                        onEnglishChange={(val) => { const n = JSON.parse(JSON.stringify(contactHeaderHist.value)); if(!n.title) n.title = {}; n.title.en = val; contactHeaderHist.update(n); }} 
                    ...
                    <IconPickerInput 
                      ...
                      onChange={(val) => { const n = JSON.parse(JSON.stringify(contactHeaderHist.value)); n.icon = val; contactHeaderHist.update(n); }} 
                    />
                  </div>
                )}
              </>
        ...
        <div className="px-1 mb-2 mt-4">
          <h3 className="text-sm font-bold text-slate-700">Contact Information List</h3>
        ...
        <div className="space-y-2">
          {(safeData.contactList || []).map((item: any, index: number) => (
            <ContactEditorItem
              key={`item-${index}`} index={index} itemData={item}
              onUpdateFull={(i: number, newD: any) => updateArrayItemFull('contactList', i, newD)}
              onRemove={() => removeArrayItem('contactList', index)}
              onMoveUp={() => moveArrayItem('contactList', index, -1)}
              onMoveDown={() => moveArrayItem('contactList', index, 1)}
              isFirst={index === 0} isLast={index === (safeData.contactList || []).length - 1}
              isExpanded={expandedItemIndex === index}
              onToggle={() => setExpandedItemIndex(expandedItemIndex === index ? -1 : index)}
            />
          ))}
