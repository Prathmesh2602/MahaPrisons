...
const ServiceCardsEditorItem = ({ index, itemData, onUpdateFull, onRemove, onMoveUp, onMoveDown, isFirst, isLast, isExpanded, onToggle }: any) => {
  const defaultItem = { title: { mr: '', en: '' }, desc: { mr: '', en: '' }, label: { mr: '', en: '' }, value: '', icon: '', image: '' };
  ...
  return (
    <div className="p-2 border border-slate-200 rounded-lg bg-slate-50 mb-3">
      <EditorBlockHeader
        title={currentItem.title?.mr || currentItem.label?.mr || `Item ${index + 1}`}
      ...
      {isExpanded && (
        <div className="p-4 space-y-4 bg-white border border-t-0 border-slate-200 rounded-b-lg">
          ...
          <div className="space-y-1">
            ...
            <input type="text" value={currentItem.image || ''} onChange={(e) => handleLocalUpdate('image', e.target.value)} placeholder="/uploads/..." className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm" />
          </div>
          <div>
...
export const ThreeColServiceCardsEditor: React.FC<ThreeColServiceCardsEditorProps> = ({ data, updateData, blockId, expandedSection }) => {
  ...
  if (activeSection === 'serviceCards') {
    return (
      <div className="pb-10 space-y-4">
        <div className="space-y-2">
        {(safeData.serviceCards || []).map((item: any, index: number) => (
          <ServiceCardsEditorItem
            key={`item-${index}`} index={index} itemData={item}
            onUpdateFull={(i: number, newD: any) => updateArrayItemFull('serviceCards', i, newD)}
            onRemove={() => removeArrayItem('serviceCards', index)}
            onMoveUp={() => moveArrayItem('serviceCards', index, -1)}
            onMoveDown={() => moveArrayItem('serviceCards', index, 1)}
            isFirst={index === 0} isLast={index === (safeData.serviceCards || []).length - 1}
            isExpanded={expandedItemIndex === index}
            onToggle={() => setExpandedItemIndex(expandedItemIndex === index ? -1 : index)}
          />
        ))}
        <button onClick={() => addArrayItem('serviceCards', { title: { mr: '', en: '' }, desc: { mr: '', en: '' }, label: { mr: '', en: '' }, value: '', icon: '', image: '' })} className="mt-4 px-4 py-3 border-2 border-dashed border-slate-300 rounded-lg text-sm font-medium text-slate-600 hover:border-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors w-full flex justify-center items-center gap-2">
