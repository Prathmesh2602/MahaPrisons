      ...
      <div className="pb-10 space-y-4">
        ...
        <div className="p-2 border border-slate-200 rounded-lg bg-slate-50 mb-4 shadow-sm">
          <EditorBlockHeader 
            ...
            isExpanded={!!expandedFixedBlocks['organization_header']} 
            onToggle={() => toggleFixedBlock('organization_header')} 
          />
          {!!expandedFixedBlocks['organization_header'] && (
            <div className="p-4 space-y-4 bg-white border-t border-slate-200 mt-2 rounded-b-lg">
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Section Title</label>
                <PhoneticInput 
                  value={safeData.sectionHeaders?.organization?.title?.mr || ''} 
                  onChange={(val) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, organization: { ...(safeData.sectionHeaders?.organization || {}), title: { ...(safeData.sectionHeaders?.organization?.title || {}), mr: val } } })} 
                  onEnglishChange={(val) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, organization: { ...(safeData.sectionHeaders?.organization || {}), title: { ...(safeData.sectionHeaders?.organization?.title || {}), en: val } } })} 
                  englishValue={safeData.sectionHeaders?.organization?.title?.en || ''} 
                />
              </div>
              <IconPickerInput 
                label="Section Icon" 
                value={safeData.sectionHeaders?.organization?.icon || ''} 
                onChange={(val) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, organization: { ...(safeData.sectionHeaders?.organization || {}), icon: val } })} 
              />
            </div>
          )}
        </div>
