    ...
    return (
      <div className="pb-10 space-y-4">
        ...
        <div className="p-2 border border-slate-200 rounded-lg bg-slate-50 mb-4 shadow-sm">
          <EditorBlockHeader 
            title="Section Header Configuration (Optional)" 
            isExpanded={!!expandedFixedBlocks['motto_header']} 
            onToggle={() => toggleFixedBlock('motto_header')} 
          />
          {!!expandedFixedBlocks['motto_header'] && (
            <div className="p-4 space-y-4 bg-white border-t border-slate-200 mt-2 rounded-b-lg">
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Section Title</label>
                <PhoneticInput 
                  value={safeData.sectionHeaders?.motto?.title?.mr || ''} 
                  onChange={(val) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, motto: { ...(safeData.sectionHeaders?.motto || {}), title: { ...(safeData.sectionHeaders?.motto?.title || {}), mr: val } } })} 
                  onEnglishChange={(val) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, motto: { ...(safeData.sectionHeaders?.motto || {}), title: { ...(safeData.sectionHeaders?.motto?.title || {}), en: val } } })} 
                  englishValue={safeData.sectionHeaders?.motto?.title?.en || ''} 
                />
              </div>
              <IconPickerInput 
                label="Section Icon" 
                value={safeData.sectionHeaders?.motto?.icon || ''} 
                onChange={(val) => handleChange('sectionHeaders', { ...safeData.sectionHeaders, motto: { ...(safeData.sectionHeaders?.motto || {}), icon: val } })} 
              />
            </div>
          )}
        </div>
