...
async function processExternalUrls(data) {
  for (const tab of data) {
    for (const item of tab.items) {
      if (item.url && item.url.startsWith('https://cdnbbsr.s3waas.gov.in/')) {
        const filename = path.basename(item.url);
        const localPath = path.join(pdfDir, filename);
        console.log(`Downloading ${filename}...`);
        try {
          await downloadFile(item.url, localPath);
          item.url = `http://localhost:5000/uploads/pdfs/${filename}`;
        } catch (e) {
...
async function reseed() {
  ...
  const oldMockData = await import(url.pathToFileURL(path.resolve(__dirname, 'old_mockData.js')).href);

  const importantLinks = mockHomepageData.important_links ? mockHomepageData.important_links.map(link => ({
    title: getBilingualText(link.title || link.text),
    href: link.href
  })) : [];

  const helplines = mockHomepageData.helpline_services ? mockHomepageData.helpline_services.map(hl => {
    let title = hl.text;
    let phone = "";
  ...
  let announcementsTabsRaw = mockHomepageData.announcements_tabs.map(tab => {
    let enTitle = "";
  ...
  const announcementsTabs = await processExternalUrls(announcementsTabsRaw);

  const heroCarouselContent = oldMockData.mockHomepageData.hero_carousel || { slides: [], directorMessage: {} };
  const ministerProfilesContent = oldMockData.mockHomepageData.minister_profiles || [];

  const blocksToSeed = [
    { type: 'hero_carousel', order: 0, content: heroCarouselContent },
    { type: 'minister_profiles', order: 1, content: ministerProfilesContent },
