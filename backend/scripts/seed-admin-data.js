...
async function main() {
  console.log("Loading administrativeData.js...");
  const modulePath = path.join(__dirname, '..', '..', 'web', 'src', 'data', 'administrativeData.js');
  const m = await import('file:///' + modulePath.replace(/\\/g, '/'));
  const data = m.administrativeData;
  
  for (const [key, pageData] of Object.entries(data)) {
    console.log(`\nProcessing ${key}...`);
    const slug = `administrative/${key}`;
    const layoutType = TEMPLATE_MAP[pageData.template] || 'standard';
