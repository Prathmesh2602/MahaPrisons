...
export const OverviewStats = ({ data: propData }) => {
  const { language } = useAccessibility();
  const data = propData || yerawadaOpenJailData.overview;
  ...
  return (
    <section data-block-type="prison_overview" className="mb-20">
      <motion.div
      ...
      >
        {data.stats?.map((stat, idx) => {
          const IconComp = stat.icon && LucideIcons[stat.icon] ? LucideIcons[stat.icon] : LucideIcons.Info;
          return (
            <motion.div
              key={idx}
