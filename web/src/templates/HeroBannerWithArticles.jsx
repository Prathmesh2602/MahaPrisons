  ...
  return (
    <div data-block-type="template" className="min-h-screen bg-[#FDFBF7] dark-mode:bg-[#1A1814] font-poppins text-[#3A332C] dark-mode:text-[#E6DFD5]">
      ...
      <div className="border-b border-[#E6DFD5] dark-mode:border-[#3A332C]">
        <div className="container mx-auto px-6 py-10 lg:py-32 flex flex-col items-center text-center max-w-4xl">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <BookOpen className="w-6 h-6 text-[#8B7355] dark-mode:text-[#C1A88B]" />
          </motion.div>
          
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-3xl md:text-5xl font-bold mb-6 tracking-tight">
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-lg italic text-[#8B7355] dark-mode:text-[#C1A88B]">
      ...
      <div className="container mx-auto px-6 py-10">
        ...
        <div className="max-w-3xl mx-auto mb-10">
          <p className="text-lg leading-relaxed text-center font-light">
            <span className="text-4xl text-[#8B7355] dark-mode:text-[#C1A88B] font-bold leading-none float-left mr-4 mt-2">
              {getTranslation(data.hero?.description).charAt(0)}
            </span>
            {getTranslation(data.hero?.description).substring(1)}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-10 items-center">
              ...
              return (
                <motion.div key={idx} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.2 }} className="group">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <h3 className="text-lg font-bold">{getTranslation(func.title)}</h3>
                  </div>
          ...
          <div className="lg:col-span-7 relative">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative z-10 p-4 bg-white dark-mode:bg-[#2A2621] shadow-2xl">
              <img src={data.hero?.heroImage} alt="Library" className="w-full h-auto object-cover grayscale-[30%] sepia-[20%]" />
            </motion.div>
        ...
        <div className="max-w-2xl mx-auto text-center font-poppins">
          <h4 className="text-sm tracking-[0.2em] uppercase text-[#8B7355] dark-mode:text-[#C1A88B] mb-6">
            {data.sectionHeaders?.stats?.title ? getTranslation(data.sectionHeaders.stats.title) : (language === 'mr' ? 'संपर्क माहिती' : 'Contact Information')}
          </h4>
...
export default HeroBannerWithArticles;
