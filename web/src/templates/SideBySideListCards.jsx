  ...
  return (
    <div data-block-type="template" className="min-h-screen bg-gray-50 dark-mode:bg-gray-900 font-poppins flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-1/2 p-6 md:p-6 lg:p-6 xl:p-24 bg-white dark-mode:bg-gray-950 text-gray-800 dark-mode:text-gray-200">
        ...
        {/* Key Features List */}
        <h3 className="text-lg font-bold text-gray-900 dark-mode:text-white mb-8 border-b-2 border-blue-500 inline-block pb-2">
          {data.sectionHeaders?.stats?.title ? getTranslation(data.sectionHeaders.stats.title) : (language === 'mr' ? 'सुविधेचे प्रकार' : 'Types of Facilities')}
        </h3>
        ...
        {/* Contact Minimal */}
        <div className="bg-gray-50 dark-mode:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark-mode:border-gray-800">
          <h4 className="text-lg font-bold text-gray-900 dark-mode:text-white mb-6">
            {data.sectionHeaders?.keyFunctions?.title ? getTranslation(data.sectionHeaders.keyFunctions.title) : (language === 'mr' ? 'संपर्क माहिती' : 'Contact Information')}
          </h4>
