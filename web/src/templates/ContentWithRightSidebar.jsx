  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] dark-mode:bg-gray-950 font-poppins py-12 md:py-10">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div data-block-type="template_hero" className="text-center mb-12">
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 dark-mode:bg-emerald-900/30 text-emerald-700 dark-mode:text-emerald-400 font-bold tracking-wide text-sm mb-6">
            {(() => {
              const IconName = data.sectionHeaders?.organization?.icon;
              const CustomIcon = IconName ? iconMap[IconName] : null;
              return CustomIcon ? <CustomIcon className="w-4 h-4" /> : <Scale className="w-4 h-4" />;
            })()}
            {data.sectionHeaders?.organization?.title ? getTranslation(data.sectionHeaders.organization.title) : (language === 'mr' ? 'डीएलएसए' : 'DLSA')}
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-[250px]">
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3 }} className="md:col-span-2 md:row-span-2 lg:col-span-2 lg:row-span-2 rounded-2xl overflow-hidden relative group shadow-lg">
            <img src={data.hero?.heroImage} alt="DLSA" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          ...
          {(data.stats || []).map((stat, idx) => {
            ...
            return (
              <motion.div key={idx} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 + (idx * 0.1) }} className="bg-white dark-mode:bg-gray-800 rounded-2xl p-6 flex flex-col justify-between shadow-lg border border-gray-100 dark-mode:border-gray-700 hover:bg-emerald-50 dark-mode:hover:bg-emerald-900/20 transition-colors">
                ...
                <div>
                  <div className="text-2xl font-black text-gray-900 dark-mode:text-white mb-1">{stat.value}</div>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.7 }} className="md:col-span-3 lg:col-span-1 rounded-2xl bg-emerald-600 p-6 text-white flex flex-col justify-between shadow-lg relative overflow-hidden">
            <div className="absolute -right-10 -top-10 opacity-10">
              <MapPin className="w-40 h-40" />
            </div>
            <h3 className="text-lg font-bold mb-4 relative z-10">{data.sectionHeaders?.stats?.title ? getTranslation(data.sectionHeaders.stats.title) : (language === 'mr' ? 'संपर्क' : 'Contact')}</h3>
